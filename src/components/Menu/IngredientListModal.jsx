...
export const IngredientListModal = ({ data, open, setOpen }) => {
    const [recipes, setRecipes] = useState(data.map(recipe => ({
        ...recipe,
        grupoIngrediente: recipe.grupoIngrediente.map(group => ({
            ...group,
            newItem: "", // Add a newItem property to each group
            item: group.item.map(item => ({ // Add fullIngredient to existing items
                ...item,
        ...
        setRecipes(recipes.map(recipe =>
            recipe._id === recipeId ? {
                ...
                grupoIngrediente: recipe.grupoIngrediente.map(group =>
                    group._id === groupId ? {
                        ...group,
                        item: group.item.filter(item => item._id !== ingredientId)
                    } : group
                )
            } : recipe
        ));
    ...
    return (
        <Modal open={open} onClose={() => setOpen(false)}>
            <Box sx={{
                width: '80vw', height: '80vh', bgcolor: 'background.paper', p: 2,
                    ...
                    <Box key={recipe._id} sx={{ mb: 3 }}>
                        ...
                        {recipe.grupoIngrediente.map(group => (
                            <Box key={group._id} sx={{ mb: 2 }}>
                                {recipe.grupoIngrediente.length > 1 ? <Typography variant="subtitle1" fontWeight="bold">{group.nombreGrupo}</Typography> : <></>}
                                <List>
                                    {group.item.map(item => (
                                        <ListItem key={item._id} secondaryAction={
                                            <IconButton edge="end" onClick={() => handleRemoveIngredient(recipe._id, group._id, item._id)}>
                                                <Delete color="error" />
                                            </IconButton>
                                        }>
                                            <ListItemText primary={item.fullIngredient} /> {/* Display fullIngredient */}
                                        </ListItem>
                                    ))}
                                </List>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    <TextField
                                        ...
                                        onChange={(e) => handleNewItemChange(recipe._id, group._id, e.target.value)}
                                    />
                                    <Button variant="contained" startIcon={<Add />} onClick={() => handleAddIngredient(recipe._id, group._id)}>
                                        Agregar
                                    </Button>
                                </Box>
                            </Box>
