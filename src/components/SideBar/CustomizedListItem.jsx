...
export const CustomizedListItem = ({ doc }) => {
    ...
    const handleClick = () => {
        ...
        if (doc.submenus.length == 0) {
            if (doc.linkTo == '/main/profile/') {
                navigate(`${doc.linkTo}${getStorageUser().username}`, { preventScrollReset: true });
            } else {
                navigate(doc.linkTo);
            }
    ...
    return (
        <div style={{ padding: 0 }}>
            <div>
                <ListItem button key={doc.id_menu} onClick={handleClick} sx={{ padding: 0 }}>
                    <Container sx={{ display: { xs: 'flex', md: 'contents' }, alignItems: { xs: 'center' }, justifyContent: { xs: 'center', md: 'unset' } }}>
